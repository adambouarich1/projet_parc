<?php
    $statutColors = [
        ...
        'depart_anticipe'  => 'bg-orange-50 text-orange-700 border-orange-200',
        'termine_attente'  => 'bg-yellow-50 text-yellow-700 border-yellow-200',
        'cloture'          => 'bg-emerald-50 text-emerald-700 border-emerald-200',
        'annule'           => 'bg-gray-100 text-gray-700 border-gray-300',
    ];
...
<div class="space-y-5 text-gray-900 font-sans">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
            <h2 class="text-2xl font-bold text-green-700 tracking-tight">Ordres de Mission</h2>
        </div>
        <div class="flex items-center gap-3">
            ...
            @if(auth()->user()->canEdit())
                <button type="button" wire:click="openCreate" class="inline-flex items-center px-9 py-3 bg-green-700 hover:bg-green-700 text-white text-sm font-semibold rounded-lg shadow-lg shadow-green-500/20 transition-all duration-200">
    ...
    @if (session()->has('error'))
        <div class="flex items-center gap-3 rounded-xl bg-red-50 border border-red-300 text-red-700 px-4 py-3">
        ...
        <div class="lg:w-64 shrink-0 w-full">
            <div class="bg-white rounded-xl p-5 border border-gray-200 shadow-sm sticky top-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button type="button" wire:click="$set('filters', { search: '', statut: '' })" class="text-xs font-medium text-green-700 hover:text-green-600 transition-colors">Réinit.</button>
                </div>
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <svg class="w-3.5 h-3.5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                            </div>
                            <input type="text" wire:model.live.debounce.300ms="filters.search"
                                class="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-300 bg-white text-gray-900 placeholder-gray-400 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all"
                                placeholder="Réf, objet, conducteur…">
                        </div>
                        <p class="text-[10px] text-gray-800 mt-1.5 leading-tight">Réf. mission · Objet · Conducteur · Marque véhicule</p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-700 block mb-2">Statut</label>
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-1.5 pt-1">
                                @foreach ([
                                    ...
                                    'en_cours'   => ['label' => 'En cours',    'active' => 'bg-green-100 text-indigo-200 border-indigo-500/40', 'inactive' => 'hover:bg-green-700/10 hover:text-green-600'],
                                ] as $key => $cfg)
                                <button type="button" wire:click="$set('filters.statut', '{{ $key }}')"
                                    class="w-full text-left px-3 py-2 rounded-lg text-sm font-bold border transition-all
                                        {{ $filters['statut'] === $key
                                            ? $cfg['active']
                                            : 'bg-gray-50 text-gray-800 border-gray-200 ' . $cfg['inactive'] }}">
                                    {{ $cfg['label'] }}
                                </button>
                                @endforeach
                            </div>
                            ...
                            <div class="pt-1" x-data="{ open: {{ in_array($filters['statut'], ['brouillon','rejete','depart_anticipe','termine_attente','cloture','annule']) ? 'true' : 'false' }} }">
                                <button type="button" @click="open = !open"
                                    class="w-full flex items-center justify-between px-3 py-1.5 rounded-lg border transition-all bg-gray-50 border-gray-200 hover:bg-gray-100">
                                    <span class="text-[10px] font-bold text-gray-700 uppercase tracking-widest">Autres statuts</span>
                                    <svg class="w-3.5 h-3.5 text-gray-700 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                </button>
                                <div x-show="open" x-transition class="flex flex-col gap-1 mt-1.5">
                                    @foreach ([
                                        'brouillon'       => ['label' => 'Brouillon',                      'active' => 'bg-gray-500/20 text-gray-200 border-gray-500/40',        'inactive' => 'hover:bg-gray-500/10 hover:text-gray-800'],
                                        ...
                                        'annule'          => ['label' => 'Annulé',                         'active' => 'bg-gray-700/60 text-gray-800 border-gray-300',         'inactive' => 'hover:bg-gray-100 hover:text-gray-800'],
                                    ] as $key => $cfg)
                                    <button type="button" wire:click="$set('filters.statut', '{{ $key }}')"
        ...
        <div class="flex-1 min-w-0 space-y-3">
            ...
            <div wire:key="mission-{{ $mission->id }}"
                ...
                <div class="pl-5 pr-5 py-5">
                    ...
                    <div class="flex flex-wrap items-center gap-3 mb-2">
                        ...
                        <span class="font-mono font-bold text-green-700 bg-green-700/15 px-3 py-1 rounded-md border border-indigo-500/25 shrink-0 leading-tight"
                        ...
                        <span class="font-bold text-gray-900 truncate leading-tight" style="font-size:23px;">
                            {{ $mission->objet }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mb-4" style="font-size:18px;">
                        ...
                        <span class="text-gray-800">{{ $mission->lieu_depart }}</span>
                        <svg class="w-4 h-4 text-gray-800 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch gap-3">
                        ...
                        <div class="flex flex-1 gap-3">
                            ...
                            <div class="flex-1 flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
                                ...
                                <div>
                                    ...
                                    <p class="font-bold text-green-700 uppercase tracking-widest leading-none mb-2" style="font-size:18px;">Conducteur</p>
                                    ...
                                    <p class="text-gray-800 mt-1" style="font-size:18px;">
                            ...
                            <div class="flex-1 flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="flex items-baseline gap-2">
                                        <span class="font-semibold text-gray-700" style="font-size:18px;">Dép</span>
                                        <span class="font-bold text-gray-900" style="font-size:23px;">{{ $mission->date_depart->format('d/m/Y') }}</span>
                                        <span class="font-mono text-gray-800" style="font-size:18px;">{{ $mission->date_depart->format('H:i') }}</span>
                                    </div>
                                    {{-- Retour — info primaire 23px --}}
                                    <div class="flex items-baseline gap-2 mt-1">
                                        <span class="font-semibold text-gray-700" style="font-size:18px;">Ret</span>
                                        <span class="font-bold text-gray-200" style="font-size:23px;">{{ $mission->date_retour_prevue->format('d/m/Y') }}</span>
                                        <span class="font-mono text-gray-800" style="font-size:18px;">{{ $mission->date_retour_prevue->format('H:i') }}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-wrap sm:flex-nowrap items-center gap-2 shrink-0 self-center">
                            ...
                            <button wire:click="openDetails({{ $mission->id }})"
                                class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-gray-800 bg-gray-700/60 hover:bg-gray-600/70 border border-gray-300 hover:border-gray-500/60 transition-all whitespace-nowrap">
            ...
            @empty
                <div class="flex flex-col items-center justify-center py-16 bg-gray-50 rounded-2xl border border-gray-200">
                    <svg class="w-14 h-14 mb-4 opacity-20 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                    <p class="text-base font-semibold text-gray-800">Aucun ordre de mission trouvé.</p>
                    <p class="text-sm mt-1 text-gray-800">Modifiez vos filtres ou créez une nouvelle mission.</p>
                </div>
            @endforelse
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" x-data x-transition.opacity>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full border border-gray-200/80 relative z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200/60 shrink-0">
                <h3 class="text-lg font-bold text-gray-900">{{ $editingId ? 'Modifier l\'ordre de mission' : 'Nouvel ordre de mission' }}</h3>
            ...
            <div class="overflow-y-auto p-6 custom-scrollbar">
                <form wire:submit.prevent="save" class="space-y-5" id="om-form">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-800">Conducteur <span class="text-rose-500">*</span></label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-gray-800">Objet de la mission <span class="text-rose-500">*</span></label>
                        <input type="text" wire:model="form.objet" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all placeholder-gray-500" placeholder="Ex: Réunion direction régionale">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea wire:model="form.description" rows="3" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all placeholder-gray-500" placeholder="Détails supplémentaires..."></textarea>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-800">Lieu de départ <span class="text-rose-500">*</span></label>
                            <input type="text" wire:model="form.lieu_depart" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all">
                            @error('form.lieu_depart') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-800">Destination <span class="text-rose-500">*</span></label>
                            <input type="text" wire:model="form.destination" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all placeholder-gray-500" placeholder="Ex: Rabat">
                            @error('form.destination') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-800">Date/heure départ <span class="text-rose-500">*</span></label>
                            <input type="datetime-local" wire:model="form.date_depart" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all [&::-webkit-calendar-picker-indicator]:filter-invert">
                            @error('form.date_depart') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-800">Date/heure retour prévue <span class="text-rose-500">*</span></label>
                            <input type="datetime-local" wire:model="form.date_retour_prevue" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all [&::-webkit-calendar-picker-indicator]:filter-invert">
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200/60 bg-gray-50 shrink-0 rounded-b-2xl">
                ...
                <button type="submit" form="om-form" class="px-5 py-2 text-sm font-semibold rounded-lg bg-green-700 text-white hover:bg-green-700 shadow-lg shadow-green-500/20 transition-all">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-3xl w-full border border-gray-200/80 relative z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200/60 shrink-0">
                <div class="flex items-center gap-3">
                    <h3 class="text-lg font-bold text-gray-900">{{ $detailMission->reference }}</h3>
            ...
            <div class="overflow-y-auto p-6 space-y-5 custom-scrollbar">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white rounded-xl p-4 border border-gray-200">
                        <p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-1">Objet</p>
                        <p class="font-semibold text-gray-900">{{ $detailMission->objet }}</p>
                        @if($detailMission->description)<p class="text-sm text-gray-700 mt-1.5">{{ $detailMission->description }}</p>@endif
                    </div>
                    <div class="bg-white rounded-xl p-4 border border-gray-200">
                        <p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-1">Trajet</p>
                        <div class="flex items-center gap-2 text-gray-900 font-semibold">
                            {{ $detailMission->lieu_depart }}
                            <svg class="w-4 h-4 text-gray-700 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white rounded-xl p-4 border border-gray-200 flex items-start gap-3">
                        <div class="p-2 bg-green-700/15 rounded-lg text-green-700 shrink-0 border border-indigo-500/20"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"/></svg></div>
                        <div><p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-0.5">Véhicule</p><p class="font-semibold text-gray-900">{{ $detailMission->vehicle->marque }} {{ $detailMission->vehicle->modele }}</p><p class="text-sm text-gray-700">{{ $detailMission->vehicle->immatriculation }}</p></div>
                    </div>
                    <div class="bg-white rounded-xl p-4 border border-gray-200 flex items-start gap-3">
                        <div class="p-2 bg-green-700/15 rounded-lg text-green-700 shrink-0 border border-indigo-500/20"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div>
                        <div><p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-0.5">Conducteur</p><p class="font-semibold text-gray-900">{{ $detailMission->driver->prenom }} {{ $detailMission->driver->nom }}</p><p class="text-sm text-gray-700">{{ $detailMission->driver->telephone }}</p></div>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white rounded-xl p-4 border border-gray-200">
                        <p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-1">Départ prévu</p>
                        <p class="font-semibold text-gray-200">{{ $detailMission->date_depart->format('d/m/Y') }}</p>
                        <p class="text-sm text-gray-700 font-mono">{{ $detailMission->date_depart->format('H:i') }}</p>
                    </div>
                    <div class="bg-white rounded-xl p-4 border border-gray-200">
                        <p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-1">Retour prévu</p>
                        <p class="font-semibold text-gray-200">{{ $detailMission->date_retour_prevue->format('d/m/Y') }}</p>
                        <p class="text-sm text-gray-700 font-mono">{{ $detailMission->date_retour_prevue->format('H:i') }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-white rounded-xl p-4 border border-gray-200"><p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-1">Km départ</p><p class="font-semibold text-gray-200">{{ number_format($detailMission->km_depart, 0, ',', ' ') }} km</p></div>
                    @if($detailMission->km_retour)
                    <div class="bg-white rounded-xl p-4 border border-gray-200"><p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mb-1">Km retour</p><p class="font-semibold text-gray-200">{{ number_format($detailMission->km_retour, 0, ',', ' ') }} km</p></div>
                    <div class="bg-green-700/10 rounded-xl p-4 border border-indigo-500/20"><p class="text-[10px] font-bold text-green-700/80 uppercase tracking-widest mb-1">Distance parcourue</p><p class="font-bold text-xl text-green-700">{{ number_format($detailMission->km_parcourus, 0, ',', ' ') }} km</p></div>
                    @endif
                </div>
                @endif
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-gray-800 border border-gray-300 shrink-0"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div>
                        <div><p class="text-xs text-gray-700">Demandé par</p><p class="text-sm font-semibold text-gray-200">{{ $detailMission->user->name }} <span class="text-gray-700 font-normal">le {{ $detailMission->created_at->format('d/m/y') }}</span></p></div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div><p class="text-xs text-gray-700">{{ $detailMission->statut === 'rejete' ? 'Rejeté par' : 'Validé par' }}</p><p class="text-sm font-semibold text-gray-200">{{ $detailMission->validator->name }} <span class="text-gray-700 font-normal">le {{ $detailMission->validated_at->format('d/m/y') }}</span></p></div>
                    </div>
                    @endif
                </div>
                ...
                <div class="bg-emerald-500/5 rounded-2xl p-5 border border-emerald-500/20 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-1.5">
                            ...
                            <input type="number" wire:model="form.km_retour" min="{{ $detailMission->km_depart }}" class="w-full rounded-lg border border-emerald-500/30 bg-gray-50 text-gray-900 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" wire:model="form.observations" class="w-full rounded-lg border border-emerald-500/30 bg-gray-50 text-gray-900 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all" placeholder="RAS, incident...">
                        </div>
                    </div>
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        <div class="fixed inset-0 bg-gray-50/80 backdrop-blur-sm" wire:click="$set('showRejectModal', false)"></div>
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-gray-200/80 relative z-10">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200/60">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-bold text-gray-900">Rejeter la mission</h3>
                </div>
                <button wire:click="$set('showRejectModal', false)" class="text-gray-700 hover:text-gray-800 p-1.5 rounded-lg hover:bg-gray-50 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="p-6 space-y-5">
                <p class="text-sm text-gray-800">Vous êtes sur le point de rejeter la mission <strong class="text-green-700 font-mono">{{ $detailMission->reference }}</strong>.</p>
                <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-800">Motif du rejet <span class="text-rose-500">*</span> <span class="text-gray-700 font-normal">(min. 10 caractères)</span></label>
                    <textarea wire:model="motifRejet" rows="3" class="w-full rounded-lg border border-gray-300 bg-white text-gray-900 text-sm focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all placeholder-gray-500" placeholder="Expliquez la raison du rejet..."></textarea>
                    @error('motifRejet') <p class="text-xs text-rose-400 mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="flex justify-end gap-3 pt-1">
                    <button wire:click="$set('showRejectModal', false)" class="px-4 py-2 text-sm font-medium rounded-lg text-gray-800 hover:bg-gray-50 border border-gray-200 transition-colors">Annuler</button>
