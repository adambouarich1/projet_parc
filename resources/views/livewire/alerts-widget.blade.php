    ...
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold text-gray-100 flex items-center gap-2">
            ...
            @if($stats['critiques'] > 0)
                <span class="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-rose-600 rounded-full animate-pulse">
                    {{ $stats['critiques'] }}
                </span>
            @endif
        </h3>
        <a href="{{ route('alerts.index') }}" class="text-xs text-indigo-400 hover:text-indigo-300">
        ...
        <div class="space-y-2">
            @foreach($alerts as $alert)
                <div class="p-3 rounded-lg border-l-4 {{ $prioriteColors[$alert->priorite] ?? 'border-l-gray-500 bg-gray-800' }}">
                    <p class="text-sm font-medium text-gray-100 truncate">{{ $alert->titre }}</p>
