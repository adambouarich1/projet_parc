...
<div class="space-y-5 text-gray-100 font-sans">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
            <h2 class="text-2xl font-bold text-white tracking-tight">Vignettes</h2>
            <p class="text-sm text-gray-400 mt-0.5">Suivi des vignettes fiscales de votre flotte.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="bg-gray-800/60 rounded-2xl border border-gray-700/50 px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest">Avec vignette</p>
        ...
        <div class="bg-gray-800/60 rounded-2xl border border-gray-700/50 px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest">Sans vignette</p>
        ...
        <div class="lg:w-64 shrink-0 w-full">
            <div class="bg-gray-800/60 rounded-xl p-5 border border-gray-700/50 shadow-lg sticky top-4">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="text-xs font-semibold text-gray-400 block mb-1.5">Année</label>
                ...
                <div class="mt-5 pt-4 border-t border-gray-700/50">
                    <p class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Coût total {{ $filters['annee'] ?: date('Y') }}</p>
            ...
            <div>
                <div class="flex items-center gap-3 mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-lg font-bold text-white">Véhicules sans vignette</h3>
                    </div>
                    ...
                    <span class="text-xs text-gray-500">pour {{ $filters['annee'] ?: date('Y') }}</span>
                </div>
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        @else
                                            <p class="text-xs text-gray-500 mt-1">Aucune vignette enregistrée</p>
                                        @endif
                                    </div>
                                </div>
            ...
            <div>
                <div class="flex items-center gap-3 mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="text-lg font-bold text-white">Véhicules avec vignette</h3>
                    </div>
                    ...
                    <div wire:key="avec-vignette-{{ $vehicle->id }}"
                        ...
                        <div class="pl-5 pr-5 py-5">
                            ...
                            <div class="flex flex-wrap items-center gap-3 mb-2">
                                ...
                                <span class="inline-flex items-center px-2.5 py-1 text-xs font-bold rounded-lg bg-indigo-500/15 text-indigo-400 border border-indigo-500/25">
                                    {{ $vignette->annee }}
                                </span>
                            ...
                            <div class="flex items-center gap-3 mb-4" style="font-size:18px;">
                                <div class="flex items-center gap-1.5 text-gray-400">
                                    ...
                                    <span class="text-gray-300">Réf. <span class="font-medium">{{ $vignette->reference_paiement }}</span></span>
                                </div>
                                @if($vignette->date_paiement)
                                <span class="text-gray-500">·</span>
                                <span class="text-gray-400">Payée le <span class="text-gray-300">{{ $vignette->date_paiement->format('d/m/Y') }}</span></span>
                                @endif
                            </div>
                            @else
                            <div class="mb-4"></div>
                                ...
                                <div class="flex flex-1 gap-3">
                                    ...
                                    <div class="flex-1 flex items-center gap-3 bg-gray-900/50 rounded-xl px-4 py-3 border border-gray-700/40">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-gray-400 mt-0.5" style="font-size:14px;">Exp. {{ $vignette->date_expiration->format('d/m/Y') }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-1 flex items-center gap-3 bg-gray-900/50 rounded-xl px-4 py-3 border border-gray-700/40">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-gray-400 mt-0.5" style="font-size:14px;">Vignette {{ $vignette->annee }}</p>
                                        </div>
                                    </div>
                ...
                <div class="flex flex-col items-center justify-center py-16 bg-gray-800/40 rounded-2xl border border-gray-700/50">
                    <svg class="w-14 h-14 mb-4 opacity-20 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <p class="text-base font-semibold text-gray-400">Aucune vignette trouvée pour {{ $filters['annee'] ?: date('Y') }}.</p>
                    <p class="text-sm mt-1 text-gray-600">Modifiez vos filtres ou enregistrez une nouvelle vignette.</p>
                </div>
                @endif
            </div>
            @endif
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" x-data x-transition.opacity>
        ...
        <div class="bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full border border-gray-700/80 relative z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-800/60 shrink-0">
                <h3 class="text-lg font-bold text-white">{{ $editingId ? 'Modifier la vignette' : 'Nouvelle vignette' }}</h3>
            ...
            <div class="overflow-y-auto p-6 custom-scrollbar">
                <form wire:submit.prevent="save" class="space-y-5" id="vignette-form">
                    ...
                    {{-- Année --}}
                    <div class="space-y-1.5">
                        <label class="text-xs font-semibold text-gray-300">Année <span class="text-rose-500">*</span></label>
                        <select wire:model.live="form.annee" class="w-full rounded-lg border border-gray-600/60 bg-gray-800/60 text-gray-100 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-300">Date expiration <span class="text-rose-500">*</span></label>
                            <input type="date" wire:model="form.date_expiration" class="w-full rounded-lg border border-gray-600/60 bg-gray-800/60 text-gray-100 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all [&::-webkit-calendar-picker-indicator]:filter-invert">
                    ...
                    <div class="flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-lg px-4 py-3">
                        ...
                        <p class="text-xs text-indigo-300">Les dates sont automatiquement remplies selon l'année sélectionnée (01/01 au 31/12).</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-300">Réf. paiement</label>
                            <input type="text" wire:model="form.reference_paiement" class="w-full rounded-lg border border-gray-600/60 bg-gray-800/60 text-gray-100 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all placeholder-gray-600" placeholder="Ex: REC-12345">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-gray-300">Date de paiement</label>
                            <input type="date" wire:model="form.date_paiement" class="w-full rounded-lg border border-gray-600/60 bg-gray-800/60 text-gray-100 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all [&::-webkit-calendar-picker-indicator]:filter-invert">
                        </div>
                        <div class="space-y-1.5">
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-800/60 bg-gray-900/50 shrink-0 rounded-b-2xl">
                ...
                <button type="submit" form="vignette-form" class="px-5 py-2 text-sm font-semibold rounded-lg bg-indigo-600 text-white hover:bg-indigo-500 shadow-lg shadow-indigo-500/20 transition-all">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full border border-gray-700/80 relative z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-800/60 shrink-0">
                <div class="flex items-center gap-3">
                    <h3 class="text-lg font-bold text-white">Vignette {{ $detailVignette->annee }}</h3>
            ...
            <div class="overflow-y-auto p-6 space-y-4 custom-scrollbar">
                ...
                <div class="bg-gray-800/60 rounded-xl p-4 border border-gray-700/50 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-100">{{ $detailVignette->vehicle->marque }} {{ $detailVignette->vehicle->modele }}</p>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="bg-indigo-500/10 rounded-xl p-4 border border-indigo-500/20 text-center">
                        <p class="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-1">Année</p>
                        <p class="font-bold text-indigo-300 text-xl">{{ $detailVignette->annee }}</p>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-gray-800/60 rounded-xl p-4 border border-gray-700/50">
                        <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Réf. paiement</p>
                        <p class="font-semibold text-gray-200 font-mono">{{ $detailVignette->reference_paiement }}</p>
                    </div>
                    @endif
                    @if($detailVignette->date_paiement)
                    <div class="bg-gray-800/60 rounded-xl p-4 border border-gray-700/50">
                        <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Date paiement</p>
                ...
                <div class="bg-gray-800/60 rounded-xl p-4 border border-gray-700/50">
                    ...
                    <p class="text-gray-300 text-sm">{{ $detailVignette->observations }}</p>
                </div>
                @endif
                ...
                <div class="flex items-center gap-3 pt-1">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold text-gray-200">{{ $detailVignette->user->name }} <span class="text-gray-500 font-normal">le {{ $detailVignette->created_at->format('d/m/Y H:i') }}</span></p>
                    </div>
                </div>
